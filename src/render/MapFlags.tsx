 ...
 * graticule, the minimum-size transforms and the magnifier all still operate on the
 ...
 * a country's cluster, the border rule measures its land, and the island floor measures
 * it again to pick candidates and again to find their neighbours. Each pass over the
 * whole world costs about 1.3 seconds, and there were four of them — the bulk of what
 * was left of the ten-second toggle.
 *
...
/**
 * How small a country's typical island must be before its islands are given a
 * rendering floor of their own, and how many of them are drawn.
 *
 * A cap, because the point is that the country is legible, not that every last islet is
 * accounted for: the Maldives read as the Maldives from their larger atolls. Ordered by
 * area, so which ones are drawn is fixed by the geography and not by anything transient.
 */
const ISLAND_LOD_MAX_FEATURE = 3
const ISLAND_LOD_COUNT = 16

/**
 * How far a country's land must be spread, relative to the size of its pieces, to
 * count as scattered.
 *
 * This is the distinction the brief draws between a country made of many islands and
 * one normal polygon with a couple of pieces beside it. Without it the rule caught
 * compact enclaves — Macau and Hong Kong against China, Saint-Pierre against Canada,
 * Guantanamo Bay inside Cuba — whose pieces sit right against a neighbour, and
 * enlarging those is exactly how a flag ends up on someone else's land. The Maldives
 * spread their atolls two hundred times their own width and Saint Vincent's islets
 * some eight times; Macau's pieces and Hong Kong's are under three times theirs.
 */
const ISLAND_LOD_MIN_SPREAD = 5

/** Outline points kept per polygon, and how far to look, when measuring clearance. */
const NEIGHBOUR_POINTS = 32
const NEIGHBOUR_SEARCH = 12

/**
 * The share of the measured clearance an island may actually grow into.
 *
 * Clearance is measured between sampled outlines, so it can overstate the true gap by
 * up to the spacing between samples. Growing into only part of it leaves headroom for
 * that error: without the margin six placements — Macau against China, Saint-Pierre
 * against Canada — crossed a border by a pixel or two.
 */
const CLEARANCE_MARGIN = 0.5

/** One island of a scattered country, with what is needed to hold it at a floor. */
export interface FlagIsland {
  key: string
  countryId: string
  iso2: string
  d: string
  centreX: number
  centreY: number
  /** Largest projected dimension, in user units. */
  size: number
  /**
   * How far the nearest other country's land is, in projected units.
   *
   * The floor may never grow an island past this, so an enlargement cannot put a
   * country's flag on someone else's territory.
   */
  clearance: number
}

/**
 * The islands of scattered countries, for the rendering floor below.
 *
 * `smallEntities.ts` already floors a feature that is too small to draw, and says
 * plainly that it cannot help here: it scales a country about a single centre, which
 * for a scattered one would push its islands apart and put them where they are not.
 * That exclusion is why an atoll nation had nothing — at world zoom a Maldivian island
 * is about a twentieth of a pixel, so its flag is drawn correctly and rasterises to
 * nothing.
 *
 * Each island is therefore floored about *its own* centre. The shape is the island's
 * real outline and the centre does not move, so nothing is displaced and nothing is
 * approximated — it is the country's own geometry, drawn no smaller than it can be
 * seen. As the camera zooms in the factor falls to 1 and the islands are drawn exactly
 * as they are.
 */
export function buildFlagIslands(
  footprints: Map<string, MultiPolygon>,
  projection: GeoProjection | null,
  codeById: (id: string) => string | undefined,
  featureById: (id: string) => Parameters<ReturnType<typeof geoPath>['area']>[0] | undefined,
  ...
  if (!projection) return []
  const path = geoPath(projection)
  const islands: FlagIsland[] = []

  /*
   * The candidates first, then the neighbours they could possibly reach.
   *
   * Order matters for speed. Building a neighbour table for the whole world means
   * projecting ring points for all 4,252 polygons — three of the ten seconds the mode
   * took to turn on — and almost none of them is within reach of a scattered
   * archipelago. Collecting the candidates first bounds that table to the polygons that
   * can actually affect an answer.
   */
  const candidates: { id: string; code: string; pieces: LandPiece[] }[] = []
  for (const id of footprints.keys()) {
    const code = codeById(id)
    if (!hasFlag(code)) continue
    const pieces = landPiecesOf(featureById(id), path, projection)
    if (pieces.length < 2) continue
    const typical = typicalLandSize(pieces)
    if (!(typical > 0) || typical > ISLAND_LOD_MAX_FEATURE) continue
    const spreadX = Math.max(...pieces.map((p) => p.x1)) - Math.min(...pieces.map((p) => p.x0))
    const spreadY = Math.max(...pieces.map((p) => p.y1)) - Math.min(...pieces.map((p) => p.y0))
    if (Math.hypot(spreadX, spreadY) / typical < ISLAND_LOD_MIN_SPREAD) continue
    candidates.push({ id, code: code as string, pieces })
  }
  if (candidates.length === 0) return islands

  /** The area any candidate could reach, as one box per candidate. */
  const reach = candidates.map(({ pieces }) => ({
    x0: Math.min(...pieces.map((p) => p.x0)) - NEIGHBOUR_SEARCH,
    y0: Math.min(...pieces.map((p) => p.y0)) - NEIGHBOUR_SEARCH,
    x1: Math.max(...pieces.map((p) => p.x1)) + NEIGHBOUR_SEARCH,
    y1: Math.max(...pieces.map((p) => p.y1)) + NEIGHBOUR_SEARCH,
  }))

  /*
   * Every country's land, boxed and outlined, so an island can be asked how much room
   * it has before it would reach somebody else — but only the land near a candidate.
   *
   * The box is only a filter. Judging clearance by boxes alone is hopeless here: one
   * polygon that crosses the antimeridian has a box spanning the whole map, which
   * overlaps nearly every island and reports no room anywhere. So boxes narrow the
   * field to the few polygons that could possibly be close, and those few are then
   * measured against their real outlines.
   */
  const neighbours: {
    id: string
    x0: number
  ...
  for (const id of footprints.keys()) {
    ...
    for (const polygon of polygons) {
      ...
      const { x0: bx0, y0: by0, x1: bx1, y1: by1 } = measured
      // Out of every candidate's reach: it can never be the nearest land to any of them.
      if (!reach.some((r) => bx0 <= r.x1 && bx1 >= r.x0 && by0 <= r.y1 && by1 >= r.y0)) continue
      const source = polygon[0]
      const stride = Math.max(1, Math.round(source.length / NEIGHBOUR_POINTS))
      const ring: [number, number][] = []
      for (let i = 0; i < source.length; i += stride) {
        const q = projection(source[i] as [number, number])
        if (q && Number.isFinite(q[0]) && Number.isFinite(q[1])) ring.push([q[0], q[1]])
      }
      if (ring.length >= 2) neighbours.push({ id, x0: bx0, y0: by0, x1: bx1, y1: by1, ring })
    }
  }

  /** Shortest distance from a set of points to a ring's segments. */
  const toRing = (points: [number, number][], ring: [number, number][]): number => {
    let best = Infinity
  ...
  const clearanceOf = (
    ...
    for (const other of neighbours) {
      ...
      const dx = Math.max(0, other.x0 - box.x1, box.x0 - other.x1)
      const dy = Math.max(0, other.y0 - box.y1, box.y0 - other.y1)
      // Boxes can only understate the true distance, so anything this far off cannot
      // be the nearest and never needs measuring.
      if (Math.hypot(dx, dy) > NEIGHBOUR_SEARCH) continue
      /*
       * Measured both ways round, and the smaller kept.
       *
       * One direction alone samples this island's outline and can step straight over a
       * boundary the two share: Macau borders China along a line that fell between two
       * samples, so it read as having room to grow and put its flag on Chinese land.
       */
      const d = Math.min(toRing(points, other.ring), toRing(other.ring, points))
      if (d < best) best = d
    }
    return best
  }

  for (const { id, code, pieces } of candidates) {
    const ordered = [...pieces].sort((a, b) => b.area - a.area).slice(0, ISLAND_LOD_COUNT)
    for (const [index, piece] of ordered.entries()) {
      const outline = path({ type: 'Polygon', coordinates: piece.polygon })
      if (!outline) continue
      const size = Math.max(piece.x1 - piece.x0, piece.y1 - piece.y0)
      if (!(size > 0)) continue
      const source = piece.polygon[0]
      const stride = Math.max(1, Math.round(source.length / NEIGHBOUR_POINTS))
      const points: [number, number][] = []
      for (let i = 0; i < source.length; i += stride) {
        const q = projection(source[i] as [number, number])
        if (q && Number.isFinite(q[0]) && Number.isFinite(q[1])) points.push([q[0], q[1]])
      }
      ...
      islands.push({
        ...
        iso2: (code as string).toLowerCase(),
        d: outline,
        centreX: (piece.x0 + piece.x1) / 2,
        centreY: (piece.y0 + piece.y1) / 2,
        size,
        clearance: clearanceOf(id, piece, points),
      })
    }
  }
...
export interface FlagIslandsProps {
  islands: FlagIsland[]
  zoomK: number
  /** Smallest an island may be drawn, in screen pixels. */
  floorPx: number
  borderColor: string
  borderWidth: number
  /**
   ...
   *
   * Set while the world is dominated. The floor still applies — a held island is still
   * grown to stay visible — but what fills it is the single world flag, so a scattered
   * archipelago shows the same continuous design as the land around it.
   */
 ...
 *
 * Drawn only while an island is below the floor; the moment its real geometry reaches
 * it, the island drops out of this layer and the country path alone draws it. Nothing
 * is hidden and no threshold removes a flag — the effect is a floor on size, never on
 * visibility. Pointer events stay off and no country id is set, so hit-testing and
 * selection continue to run against the country path underneath.
 */
export const FlagIslands = memo(function FlagIslands({
  islands,
  zoomK,
  floorPx,
  borderColor,
  ...
  /*
   * How far each island may actually be grown.
   *
   * The floor wants `floorPx / drawn size`; the geography allows only as much as fits
   * in the water around the island. Guantanamo Bay is the case that forces the second
   * limit — it is an enclave inside Cuba, so growing it to the floor painted the
   * American flag onto Cuban land. Where the room is not there, the island is drawn at
   * its true size and no flag leaves the territory it belongs to.
   */
  const held = islands.flatMap((island) => {
    if (!flags[island.iso2]) return []
    const drawn = island.size * zoomK
    if (drawn >= floorPx) return []
    const wanted = floorPx / drawn
    const allowed = 1 + (2 * island.clearance * CLEARANCE_MARGIN) / island.size
    const scale = Math.min(wanted, allowed)
    return scale > 1 ? [{ island, scale }] : []
  })
  if (held.length === 0) return null

  return (
    <g className="map-flag-islands" pointerEvents="none">
      {held.map(({ island, scale }) => {
        return (
          <path
            key={island.key}
            d={island.d}
